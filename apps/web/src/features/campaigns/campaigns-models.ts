...
export const campaignModel = {
  async retrieveCampaignFromDatabaseById(id: string): Promise<SelectCampaign | null> {
    const db = getDb();
  ...
  async retrieveCampaignFromDatabaseBySlug(slug: string): Promise<SelectCampaign | null> {
    const db = getDb();
    const result = await db
      .select()
      .from(campaign)
  ...
  async retrieveCampaignsFromDatabaseByOrganization(
  ...
  async retrieveCampaignCountFromDatabaseByOrganizationAndFilters(
    organizationId: string,
  ...
  async retrieveCampaignsWithDonationStatsFromDatabaseByOrganizationAndFilters(
    organizationId: string,
    filters: CampaignFilters = {},
  ) {
    const db = getDb();
  ...
  async retrieveCampaignWithDonationStatsFromDatabaseById(
    campaignId: string,
    organizationId: string,
  ) {
    const db = getDb();
  ...
  async getIsSlugAvailableInDatabase(
    organizationId: string,
    slug: string,
  ...
  async saveCampaignToDatabase(data: InsertCampaign): Promise<SelectCampaign | null> {
    const db = getDb();
    const [result] = await db.insert(campaign).values(data).returning();
    return result ?? null;
  },

  async updateCampaignInDatabase(
    id: string,
    organizationId: string,
  ...
  async updateCampaignStatusInDatabase(
    id: string,
    organizationId: string,
    status: SelectCampaign["status"],
  ): Promise<SelectCampaign | null> {
    return this.updateCampaignInDatabase(id, organizationId, { status });
  },

  async publishCampaignInDatabase(
    id: string,
    organizationId: string,
  ): Promise<SelectCampaign | null> {
    const db = getDb();
  ...
  async deleteCampaignInDatabase(id: string, organizationId: string): Promise<boolean> {
    const db = getDb();
  ...
  async retrieveCampaignCountFromDatabaseByOrganization(organizationId: string): Promise<number> {
    const db = getDb();
    const result = await db
      .select({
        count: count(),
  ...
  async getHasDonationsInDatabase(campaignId: string): Promise<boolean> {
    const db = getDb();
    const result = await db
      .select({ count: count() })
      .from(donation)
  ...
  async retrievePublicCampaignFromDatabaseBySlug(slug: string) {
    const db = getDb();
    const now = new Date();
  ...
  async retrieveRecentDonationsFromDatabaseByCampaign(campaignId: string, limit: number = 20) {
    const db = getDb();
    const result = await db
      .select({
        id: donation.id,
  ...
  async retrieveCampaignForPreviewFromDatabaseById(campaignId: string) {
    const db = getDb();
    const result = await db
      .select({
        id: campaign.id,
...
export const categoryModel = {
  async retrieveEnabledCategoriesFromDatabase(): Promise<SelectCategory[]> {
    const db = getDb();
    return await db
      .select()
      .from(category)
  ...
  async retrieveCategoryFromDatabaseById(id: string): Promise<SelectCategory | null> {
    const db = getDb();
    const result = await db.select().from(category).where(eq(category.id, id));
    return result[0] ?? null;
  },

  async retrieveAllCategoriesFromDatabase(): Promise<SelectCategory[]> {
    const db = getDb();
...
export const donationModel = {
  async retrieveDonationFromDatabaseById(id: string): Promise<SelectDonation | null> {
    const db = getDb();
    const result = await db.select().from(donation).where(eq(donation.id, id));
    return result[0] ?? null;
  },

  async retrieveDonationFromDatabaseByReference(reference: string): Promise<SelectDonation | null> {
    const db = getDb();
    const result = await db.select().from(donation).where(eq(donation.reference, reference));
    return result[0] ?? null;
  },

  async retrieveDonationsFromDatabaseByCampaign(campaignId: string): Promise<SelectDonation[]> {
    const db = getDb();
    return await db
      .select()
      .from(donation)
  ...
  async retrieveDonationsFromDatabaseByDonor(donorId: string): Promise<SelectDonation[]> {
    const db = getDb();
    return await db
      .select()
      .from(donation)
  ...
  async retrieveSuccessfulDonationsFromDatabaseByCampaign(
    campaignId: string,
  ...
  async saveDonationToDatabase(data: InsertDonation): Promise<SelectDonation | null> {
    const db = getDb();
    const [result] = await db.insert(donation).values(data).returning();
    return result ?? null;
  },

  async updateDonationInDatabase(
    id: string,
