...
import type { DonationFilters, CreateDonation } from "./donations-schemas";
...
export const donationModel = {
  async retrieveDonationsFromDatabaseByOrganization(
    ...
    if (dateFrom) {
      conditions.push(gte(donation.createdAt, dateFrom));
    }

    if (dateTo) {
      conditions.push(lte(donation.createdAt, dateTo));
    }
